...
|---|---|---|---|
| **Primär — Lesetext, Zitate, Protokolle** | **Lora** | Lokal (@fontsource/lora) | Humanistische Serifenschrift, entwickelt für Bildschirmlesen. Warm, buchförmig, mit archivarischem Charakter — ohne historisierende Karikatur. |
| **Sekundär — Navigation, Labels, UI** | **Inter** | Lokal (@fontsource/inter) | Neutrale, hoch optimierte Interface-Schrift. Klare Hierarchie, ausgezeichnete Lesbarkeit bei kleinen Größen. Ergänzt Lora, konkurriert nicht. |
...
### Neue Zustände: GapInline (Feature A)

Unleserliche Textstellen in Transkriptionen erhalten zwei definierte Zustände.
Diese ergänzen die bestehenden Hover/Fokus/Skeleton-Regeln — kein Konflikt.

```css
/* Unleserliche Stelle — offen, klickbar */
.gap-inline {
...
/* Unleserliche Stelle — Beitrag bereits eingereicht */
.gap-inline.gap-submitted {
  background: rgba(59, 110, 74, 0.15);  /* --color-success gedimmt */
  /* kein Streifen-Muster mehr — Beitrag ist eingegangen */
  cursor: default;
}

.gap-inline.gap-submitted::after {
  content: " ✓";
  font-size: 0.75em;
...
- `.gap-inline` nutzt bewusst `--color-stub` (#A09890) — dieselbe Farbe wie
  nicht-öffentliche Inhalte. Das ist kein Zufall: beides sind Lücken, beides
  sind Einladungen. Die visuelle Sprache ist konsistent.
- `.gap-submitted` wechselt zu einem sehr zurückhaltenden Grün — kein Jubel,
  nur eine ruhige Bestätigung. Das entspricht dem Tonalitätsprinzip aus
  T1-emotionaler-kern.md: Ehrlich, nicht enthusiastisch überladen.
- Das `✓`-Zeichen nach Einreichung ist bewusst klein und nachgestellt —
  es signalisiert Status, nicht Erfolg.
