    ...
    def extract_pages(
        ...
        try:
            if output_format == "pdf":
                ...
                try:
                    reader = PdfReader(
                        str(pdf_path_obj), strict=False
                    )  # strict=False für fehlertolerantes Lesen
                ...
                for page_num in range(total_pages):
                    try:
                        ...
                        raise Exception(
                            f"Fehler beim Extrahieren von Seite {page_num + 1}: {e}"
                        ) from e
                ...
                if file_size_mb > 50:
                    ...
                    logging.info(
                        f"Große Datei erkannt ({file_size_mb:.2f} MB), verwende DPI {dpi}"
                    )
                elif file_size_mb > 20:
                    dpi = 200
                    logging.info(
                        f"Mittlere Datei erkannt ({file_size_mb:.2f} MB), verwende DPI {dpi}"
                    )
                ...
                if settings.poppler_path:
                    ...
                    logging.warning(
                        "POPPLER_PATH nicht konfiguriert. Versuche Poppler aus PATH zu verwenden."
                    )
                ...
                try:
                    ...
                    logging.warning(
                        f"Konnte Seitenzahl nicht ermitteln: {e}, verarbeite alle Seiten auf einmal"
                    )
                    total_pages = None
                ...
                if total_pages and total_pages > batch_size:
                    logging.info(
                        f"Große PDF ({total_pages} Seiten), verwende Batch-Verarbeitung (Batch-Größe: {batch_size})"
                    )
                    pages_info = []
                    ...
                    for batch_start in range(1, total_pages + 1, batch_size):
                        ...
                        logging.info(
                            f"Verarbeite Seiten {batch_start}-{batch_end} von {total_pages}"
                        )
                        ...
                        try:
                            ...
                            for idx, image in enumerate(images):
                                ...
                                try:
                                    relative_path = str(
                                        page_path.relative_to(Path.cwd())
                                    )
                                except ValueError:
                                    relative_path = str(page_path)
                            ...
                            logging.error(
                                f"Fehler beim Verarbeiten von Seiten {batch_start}-{batch_end}: {e}"
                            )
                            raise
                else:
            ...
            raise Exception(f"Fehler beim Extrahieren der PDF-Seiten: {e}") from e
