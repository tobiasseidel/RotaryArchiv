...
class OCRPipeline:
    ...
    async def process_document(
        ...
        tesseract_task = asyncio.to_thread(
            self.tesseract.extract_text,
            file_path,
            language
        )
        ollama_task = asyncio.to_thread(
            self.ollama_vision.extract_text,
            file_path
        )
        ...
        if use_correction and tesseract_text and ollama_text:
            ...
            comparison_result = await asyncio.to_thread(
                self.ollama_gpt.compare_ocr_results,
                tesseract_text,
                ollama_text
            )
        ...
        if use_correction and final_text:
            correction_result = await asyncio.to_thread(
                self.ollama_gpt.correct_ocr_errors,
                final_text
            )
