...
from src.rotary_archiv.api.schemas import EntityCreate, EntityResponse, EntityUpdate
...
def create_entity(entity: EntityCreate, db: Session = Depends(get_db)):
    ...
    existing = (
        db.query(Entity)
        .filter(Entity.name == entity.name, Entity.entity_type == entity.entity_type)
        .first()
    )

    if existing:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Entität existiert bereits"
        )
    ...
    db_entity = Entity(
        ...
        wikidata_description=wikidata_data.get("description")
        if wikidata_data
        else None,
...
def get_entity(entity_id: int, db: Session = Depends(get_db)):
    ...
    if not entity:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Entität nicht gefunden"
        )
    return entity
...
def update_entity(
    ...
    if update_data.get("wikidata_id"):
        matcher = WikidataMatcher()
